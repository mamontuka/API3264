...
async def get_or_create_qwen_chat(token_obj, openweb_chat_id: str, model: str):
    ...
    # 🔥 FIX: Задержка ВЫНЕСЕНА за пределы lock
...
def _get_openwebui_chat_id_from_db(user_id: str, conversation_title: Optional[str] = None) -> Optional[str]:
    ...
    Table: 'chat', Columns: id, user_id, title, updated_at
    """
...
def _generate_openweb_chat_id(request: Request, body: Dict[str, Any], model: str) -> str:
    ...
    1. Explicit conversation_id/chat_id from body/headers
    2. ID from OpenWebUI PostgreSQL DB (table 'chat')
    3. STABLE hash based on user_id + model + hour (DEFAULT)
    4. Fallback: random UUID
    """
        ...
        try:
            async with http_client.stream(
                ...
                if actual_status and actual_status >= 400:
                    ...
                    # 🔥 ОПТИМИЗИРОВАННАЯ ОБРАБОТКА: "The chat is in progress!"
                    is_chat_in_progress = "chat is in progress" in error_details
                    
                    if is_chat_in_progress:
                        # 🔥 РАЗУМНЫЕ задержки: 30s → 45s → 60s = 135s суммарно (~2 минуты)
                        chat_in_progress_max_retries = 2  # Всего 3 попытки: 0,1,2
                    ...
                    # Обычная ошибка 400/500 — короткий ретрай
...
async def _stream_openai_response(token_info, chat_id: str, payload: Dict[str, Any], model: str, openweb_chat_id: str):
    """Streaming response in OpenAI-compatible SSE format - FIXED"""
    ...
    try:
        while True:
            # 🔥 FIX: Проверяем завершение задачи
            if task.done():
                # Вычитываем остатки из очереди
                while not queue.empty():
                    try:
                        chunk = queue.get_nowait()
                        has_streamed_chunks = True
                        # 🔥 FIX: Правильный SSE-формат: " " + JSON + "\n\n"
                ...
                # Если задача завершилась с ошибкой и мы ещё не стримили — отправляем ошибку
        ...
        # 🔥 FIX: Если задача завершилась успешно, но мы ещё не стримили — отправляем контент
        ...
        # 🔥 FIX: Правильный финальный чанк с " "
        yield "data: " + json.dumps({
            "id": "chatcmpl-stream",
            "object": "chat.completion.chunk",
            "created": int(time.time()),
