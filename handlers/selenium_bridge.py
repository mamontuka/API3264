...
class SeleniumBridge:
    ...
    async def upload_file_and_wait(self, filepath: Path, prompt_text: str = "", chat_id: str = None, target_model: str = None) -> str:
        """
        ...
        Args:
            ...
            chat_id: Optional Qwen chat ID to navigate to before uploading
            target_model: Model name to select in Qwen UI (e.g., "qwen-vl-max")
        """
        try:
            ...
            if chat_id:
                ...
                if target_model:
                    ...
                    try:
                        # Click on the model selector (usually this is a button with the model name)
            ...
            async (args) => {
                ...
                // Dispatch to active element (textarea)
            ...
            result = await self.page.evaluate(paste_script, {
            ...
            # 5. We are waiting for the preview image to appear in the UI.
            ...
            if not preview_found:
                ...
                screenshot_path = Config.TEMP_FILES_DIR / f"debug_paste_{uuid.uuid4().hex[:8]}.png"
                await self.page.screenshot(path=str(screenshot_path))
                logger.debug(f"📸 Debug screenshot: {screenshot_path}")
            ...
            # 🔥 POLLING CYCLE WITH DEADLINE CHECK
            while asyncio.get_event_loop().time() < deadline:
                ...
                for selector in Config.QWEN_RESPONSE_SELECTORS:
                    try:
                        ...
                        if await locator.is_visible():
                            ...
                            if text and len(text.strip()) > 5:  # Ignore empty/short blocks
                                ...
                                await asyncio.sleep(3.0)  # We give time for full generation
                                
                                # Let's check if the text has stopped changing
                                prev_text = text
                                for _ in range(3):  # 3 checks with an interval of 1 second
                                    await asyncio.sleep(1.0)
                                    current_text = await locator.inner_text()
                                    if current_text == prev_text:
                                        break
                                    prev_text = current_text
                                    logger.debug(f" Text still changing... ({len(current_text)} chars)")
                ...
                # Fallback: Check for any new text in the last message
                try:
                    ...
                    if await last_msg.is_visible():
                        ...
                        if text and len(text.strip()) > 10:
                            ...
                            # 🔥 We are also waiting for the completion of the fallback.
                ...
                # A short pause before the next check
            ...
            if not response_found:
                ...
                screenshot_path = Config.TEMP_FILES_DIR / f"debug_timeout_{uuid.uuid4().hex[:8]}.png"
                try:
                    await self.page.screenshot(path=str(screenshot_path), full_page=True)
                    logger.error(f"📸 Timeout screenshot saved: {screenshot_path}")
                except Exception as se:
                    logger.warning(f"⚠️ Failed to save screenshot: {se}")
                
                raise TimeoutError(f"Vision response timeout after {timeout_sec}s. No valid response detected.")

            # We give time for full generation
            if result_text:
                await asyncio.sleep(2.0)
                logger.info(f"✅ Received response ({len(result_text)} chars)")
                return result_text
            else:
                raise TimeoutError("Response text is empty")
